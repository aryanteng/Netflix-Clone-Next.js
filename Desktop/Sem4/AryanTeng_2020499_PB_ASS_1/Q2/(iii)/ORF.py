...
def findORF(frame):
    ...
    for j in range(0,len(input)):
        if input[j] == 'ATG':
            ...
            length += 1
            if exonFound == 1 and length >= 10: 
            #Since it is given in question that threshold value is 30 nucleotides, that means that the length would be greater than or equal to 10 since a single element has 3 letters.
                orfCounter += 1
                exonFound = 0
                length = 0
            else:
